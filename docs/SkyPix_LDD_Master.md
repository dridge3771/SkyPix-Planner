...
LDD VERSION: 2026-03-29-A  |  PARTS: XL–LVII  |  VERIFY: BROADBAND-PATH-LOCKED
...
Algorithm:
Scan HA −12h → +12h in N_STEPS=720 steps (2-min resolution)
At each step: compute obj_alt (Bennet refraction) and horizon_alt (SLERP)
Detect ALL sign changes of (obj_alt − horizon_alt):
negative → positive = rising crossing
positive → negative = setting crossing
Pair each rising crossing with the next setting crossing
Discard unpaired crossings at ±12h boundary
Return list of complete [ha_rise, ha_set] pairs
...
Split at meridian: east=[hr, min(hs,0)], west=[max(hr,0), hs]
Apply zenith obstruction clipping to overlapping component
Discard segments < min_visible_time_hr (default 1.0h)
Accumulate into visible_east, visible_west
Output:
